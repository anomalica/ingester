...
- creators: the document's human author(s), one named person per list item. Omit creators entirely when the author is an organisation, agency, military unit, or office rather than a named person (e.g. "89 ATKS", "Department of Defense") - never put a non-person there. Two forms for a person whose NAME you do not have: if the page only DESCRIBES the author (real name not given, e.g. "Senior US Intelligence Officer"), bracket the description in lower case - creators: ["[senior US intelligence officer]"] - the brackets mean "a description, not a name", so a consumer never builds a person from it. Bracket ONLY a value that is ENTIRELY a description with no name or pseudonym in it at all. A pseudonym or a lone given name IS a name - keep it verbatim, unbracketed ("Dr. X", "Sally"), and so is a name WITH a parenthetical qualifier ("Sally (Budd Hopkins abductee)", "Dr. X (French physician)"): a name plus a disambiguating parenthetical is a person, not a description. Only when there is no name or pseudonym at all - a pure role or title - does it get brackets. If the author's name is WITHHELD or REDACTED on the page, write creators: ["[redacted]"] - NOT omitted, because that records a name was given and withheld. NEVER put {{{{redacted}}}} or any {{{{...}}}} marker in creators or any other frontmatter value: the {{{{...}}}} grammar is for the body only, and a withheld author is the bracketed [redacted].
...
- Classification markings (for declassified government documents):
  ...
  - Per-portion markings that DIFFER from the overall banner (paragraph/section prefixes like "(U)", "(S//REL)", "(S/RELIDO)") become an inline annotation at the start of the portion they govern: {{{{classification: U}}}}, {{{{classification: S//REL}}}}, {{{{classification: "S/RELIDO"}}}}. Quote the value if it contains a colon or comma. The marking applies from its position until the next classification marking. EXCEPTION - a portion marking that is STRUCK THROUGH: the strike wins over the portion-marking role exactly as it wins over the document banner. A struck "(S/RELIDO)" or "(SECRET//REL TO USA, FVEY)" at the head of a paragraph is being DECLASSIFIED, not governing the portion, so transcribe it as struck text (~~(S/RELIDO)~~) in reading order per the strikethrough rule - NEVER as a {{{{classification}}}} annotation. Only an UNSTRUCK portion marking becomes a classification annotation.
  - Reproduce marking values verbatim (minus parens). Do not normalise or expand them.
  - Do NOT INVENT a strikethrough for a classification marking that is merely PRINTED and not struck - strikethrough (~~text~~) is only for text genuinely crossed out. But when a banner IS genuinely struck through in the source (a declassification stroke ruled through it), the strike is the event: transcribe it as struck text (~~(SECRET//REL TO USA, FVEY)~~) in reading order per the strikethrough rule above - NOT as a live {{{{classification}}}} annotation and NOT copied into the frontmatter classification field. A struck banner records that a marking was REMOVED, not one in force; the classification tags and frontmatter classification are for markings that still stand.
